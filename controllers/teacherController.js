...
const teacherController = {
    async getTeacherById(req, res) {
        try {
            ...
            if (!teacherById) return res.status(404).json({ message: 'Teacher not found' });
    ...
    async getTeacherByCurrentUser(req, res) {
        try {
            ...
            res.status(500).json({ message: 'Error retrieving teacher by user ID', error });
        }
    },
    async createTeacher(req, res) {
        try {
            ...
            const createdTeacher = await teacher.createTeacher(username, password, role_id, scheduleDate);
