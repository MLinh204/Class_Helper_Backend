...
const vocabListController = {
    ...
    async getVocabListById(req, res) {
        try {
            ...
            if (!vocabListById) return res.status(404).json({ message: 'Vocab list not found' });
    ...
    async createVocabList(req, res) {
        try {
            ...
            if (!teacherByUserId) return res.status(404).json({ message: 'Teacher not found' });

            const { title, description, category } = req.body;
            const word_count = 0;
            const createdAt = new Date();
            const vocabList = new VocabList();
            const newVocabList = await vocabList.createVocabList(title, description, category, word_count, teacherByUserId.id, createdAt);
            res.status(200).json(newVocabList);
        } catch (err) {
            res.status(500).json({ message: 'Error creating vocab list', error: err.message });
        }
    },
    async updateVocabList(req, res) {
        try {
            ...
            const { title, description, category } = req.body;
            const updatedVocabList = await vocabList.updateVocabList(id, title, description, category);
    ...
    async increaseWordCount(req, res) {
        try {
            ...
            const vocabList = new VocabList();
            const updatedVocabList = await vocabList.updateWordCount(list_id, word_count);
            res.status(200).json(updatedVocabList);
        } catch (error) {
