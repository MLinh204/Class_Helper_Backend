...
const vocabController = {
  async createVocab(req, res) {
    try {
      ...
      const newVocab = await vocab.createVocab(
        list_id,
        word,
        translation,
        definition,
      ...
      const updatedStudent = await calculateAndUpdatePoints(
        studentId,
        pointToAdd
      );
    ...
    try {
      ...
      if (!isCreatedByTeacher)
        return res.status(403).json({
          message:
            "Only special students can create vocabulary with this button",
        });
      const studentId = studentByUserId.id;
      const createdAt = new Date();
      const vocab = new Vocab();
      ...
      if (!wordInDictionary.data || wordInDictionary.data.length === 0) {
        return res
          .status(404)
          .json({ message: "Word not found in dictionary" });
      }
      const definition =
        wordInDictionary.data[0].meanings[0].definitions[0].definition;
      const part_of_speech = wordInDictionary.data[0].meanings[0].partOfSpeech;
      const example_sentence =
        wordInDictionary.data[0].meanings[0].definitions[0].example ||
        "Not available";
      const synonyms = wordInDictionary.data[0].meanings[0].synonyms
        ? wordInDictionary.data[0].meanings[0].synonyms.join(", ")
        : "Not available";
      const antonyms = wordInDictionary.data[0].meanings[0].antonyms
        ? wordInDictionary.data[0].meanings[0].antonyms.join(", ")
        : "Not available";
      const newVocab = await vocab.createVocab(
        list_id,
        word,
      ...
      res.status(500).json({
        message: "Error creating vocabulary with API",
        error: err.message,
      });
    }
  },
  async getVocabById(req, res) {
    try {
      ...
      res.status(500).json({
        message: "Error retrieving vocabulary by id",
        error: err.message,
      });
    }
  },
  async getVocabByListId(req, res) {
    try {
      ...
      res.status(500).json({
        message: "Error retrieving vocabulary by list id",
        error: err.message,
      });
    }
  },
  async updateVocab(req, res) {
    try {
      ...
      const vocab = new Vocab();
      const updatedVocab = await vocab.updateVocab(
        id,
        word,
        translation,
        definition,
  ...
  async searchVocab(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error searching vocabulary", error: err.message });
    }
  },
  async sortVocabsBy(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error sorting vocabulary", error: err.message });
    }
  },
};
