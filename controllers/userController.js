...
const userController = {
    ...
    async deleteUser(req, res){
        try{
            ...
            if(!deletedUser.affectedRows) return res.status(404).json({ message: 'User not found' });
    ...
    async getUserByUsername(req, res){
        try{
            ...
            if(!userByUsername) return res.status(404).json({ message: 'User not found' });
            res.json(userByUsername);
        } catch (err){
            res.status(500).json({ message: 'Error retrieving user by username', err});
        }
    },
