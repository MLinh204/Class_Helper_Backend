...
const attendanceListController = {
  async createAttendanceList(req, res) {
    try {
      ...
      const newAttendanceList = await attendanceList.createAttendanceList(
        title,
        teacherId,
        status,
        createdAt
      ...
      setTimeout(
        async () => {
          try {
            ...
            await attendanceList.updateAttendanceList(
              newAttendanceList.id,
              newAttendanceList.title,
              "closed"
            );
            console.log(
              `Attendance list ${newAttendanceList.id} closed and pending records marked absent`
            );
          } catch (err) {
            console.error(
              "Error closing attendance list and marking pending records absent",
              err
            );
          }
        },
      ...
      res
        .status(500)
        .json({ message: "Error creating attendance list", error: e.message });
    }
  },
  async getAttendanceListsByTeacherId(req, res) {
    try {
      ...
      const attendanceLists =
        await attendanceList.getAttendanceListsByTeacherId(teacherId);
      res.status(200).json(attendanceLists);
    } catch (error) {
      res
        .status(500)
        .json({ message: "Error retrieving attendance lists", error });
    }
  },
  async getAttendanceListById(req, res) {
    try {
      ...
      const existingAttendanceList =
        await attendanceList.getAttendanceListById(id);
      ...
      res
        .status(500)
        .json({ message: "Error retrieving attendance list", error });
    }
  },
  ...
  async updateAttendanceList(req, res) {
    try {
      ...
      if (existingAttendanceList.status === "closed")
        return res
          .status(400)
          .json({ message: "Attendance list is closed, cannot update" });
      const updatedAttendanceList = await attendanceList.updateAttendanceList(
        id,
        title,
        status
      );
      ...
      res
        .status(500)
        .json({ message: "Error updating attendance list", error });
    }
  },
  async deleteAttendanceList(req, res) {
    try {
      ...
      if (!existingAttendanceList)
        return res.status(404).json({ message: "Attendance list not found" });
      const deletedAttendanceList =
        await attendanceList.deleteAttendanceList(id);
      res.status(200).json(deletedAttendanceList);
    } catch (error) {
      res
        .status(500)
        .json({ message: "Error deleting attendance list", error });
    }
  },
  async searchAttendanceList(req, res) {
    try {
      ...
      res
        .status(500)
        .json({ message: "Error searching attendance lists", error });
    }
  },
  async sortAttendanceLists(req, res) {
    try {
      ...
      const attendanceList = new AttendanceList();
      const sortedLists = await attendanceList.sortAttendanceList(
        column,
        order
      );
      ...
      res
        .status(500)
        .json({ message: "Error sorting attendance lists", error });
    }
  },
};
