...
const studentController = {
    ...
    async getStudentById(req, res) {
        try {
            ...
            if (!studentById) return res.status(404).json({ message: 'Student not found' });
    ...
    async getStudentByUserId(req, res) {
        try {
            ...
            if (!studentByUserId) return res.status(404).json({ message: 'Student not found' });
            res.json(studentByUserId);
        } catch (error) {
            res.status(500).json({ message: 'Error retrieving student by user id', error });
        }
    },
    ...
    async addPoint(req, res) {
        try {
            ...
            const cumulativePoints = (existingUser.level - 1) * 2000 + existingUser.point;
            ...
            if (newCumulativePoints < 0) {
                return res.status(400).json({ message: 'Not enough points. Level cannot be less than 1' });
            }
            ...
            if (newLevel > existingUser.level) {
                ...
                updatedStudent = await student.updateStudentPoint(id, newLevel, newPoints);
                return res.json({ message: `Student increased ${levelIncrease} level(s)`, updatedStudent });
            } else if (newLevel < existingUser.level) {
                const levelDecrease = existingUser.level - newLevel;
                updatedStudent = await student.updateStudentPoint(id, newLevel, newPoints);
                return res.json({ message: `Student decreased ${levelDecrease} level(s)`, updatedStudent });
            } else {
                // No level change; only update the points.
                updatedStudent = await student.updateStudentPoint(id, existingUser.level, newPoints);
                return res.json({ message: 'Student point updated successfully', updatedStudent });
            }
        } catch (error) {
    ...
    async modifyHeart(req, res) {
        try {
            ...
            return res.json({ message: 'Student heart updated successfully', updatedStudent });
        } catch (error) {
    ...
    async modifyLevel(req, res) {
        try {
            ...
            return res.json({ message: 'Student level updated successfully', updatedStudent });
        } catch (error) {
